...
impl Expr {
    ...
    pub fn as_expr(pair: Pair<Rule>) -> Expr {
        ...
        match inner_rules.next() {
            Some(expr_pair) => {
                let expr: Expr = Self::as_expr(expr_pair.clone());
    ...
    fn as_div_expr(pair: Pair<Rule>) -> Expr {
        ...
        match inner_rules.next() {
            Some(div_pair) => {
                let div_expr: Expr = Self::as_div_expr(div_pair.clone());
                Expr::Divide(Box::new(factor), Box::new(div_expr))
            },
            None => factor 
        }
    }
    ...
    fn as_factor(pair: Pair<Rule>) -> Expr {
        ...
        match inner_rule.as_rule() {
            ...
            Rule::expr => Self::as_expr(inner_rule),
            _ => unreachable!(),
        }
    }
